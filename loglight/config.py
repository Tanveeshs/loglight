...
class LoggerConfig:
    ...
    redaction_rules: list = (
        None  # list of field names to redact (deprecated, use masking options)
    )
    ...
    masking_strategy: str = (
        "FULL"  # Masking strategy: FULL, PARTIAL, HASH, NULLIFY, FIRST_LAST
    )
    ...
    def __post_init__(self):
        ...
        self.include_timestamp = (
            os.getenv("LOG_INCLUDE_TIMESTAMP", str(self.include_timestamp)).lower()
            == "true"
        )
        ...
        try:
            self.sampling_rate = float(
                os.getenv("LOG_SAMPLING_RATE", self.sampling_rate)
            )
        except ValueError:
        ...
        self.enable_field_masking = (
            os.getenv("LOG_ENABLE_MASKING", str(self.enable_field_masking)).lower()
            == "true"
        )
        self.masking_strategy = os.getenv(
            "LOG_MASKING_STRATEGY", self.masking_strategy
        ).upper()
        self.enable_pattern_matching = (
            os.getenv(
                "LOG_ENABLE_PATTERN_MATCHING", str(self.enable_pattern_matching)
            ).lower()
            == "true"
        )
        self.enable_nested_masking = (
            os.getenv(
                "LOG_ENABLE_NESTED_MASKING", str(self.enable_nested_masking)
            ).lower()
            == "true"
        )
        self.enable_value_masking = (
            os.getenv(
                "LOG_ENABLE_VALUE_MASKING", str(self.enable_value_masking)
            ).lower()
            == "true"
        )

        try:
            self.partial_keep_chars = int(
                os.getenv("LOG_PARTIAL_KEEP_CHARS", self.partial_keep_chars)
            )
        except ValueError:
